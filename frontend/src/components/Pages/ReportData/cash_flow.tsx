...
function filterCashflowData(
  ...
  for (const [key, val] of Object.entries(data)) {
    ...
    if (CASHFLOW_DATASET.some((kw) => k.includes(kw))) {
      result[key] = val;
    }
  }
...
function DetailPanel({
  ...
  // Filter to cashflow-only keys
  const cashflowData = record.data ? filterCashflowData(record.data as Record<string, unknown>) : {};
  const dataEntries = Object.entries(cashflowData);
