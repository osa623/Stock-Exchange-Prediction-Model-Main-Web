...
import { incomeDataByYear, Year, IncomeRow, years as staticYears } from "./incomeData";
...
export default function IncomeST({ symbol }: Props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setApiError(false);
    try {
      const records = await getRecordsByCompanyAndType(symbol, "income");
  ...
  const rowsForYear: IncomeRow[] =
    apiRows[activeYear] ??
    (incomeDataByYear as Record<string, IncomeRow[]>)[activeYear] ??
    [];
