...
export default function FinancialPositionSubNav() {
  ...
  const items = [
    { name: "Financials", path: "/report_data/financial_position" },
    { name: "Graphs", path: "/report_data/financial_position/graphs" },
  ];
    ...
    <nav className="flex justify-center w-full mb-4">
      <div className="w-full max-w-[95vw] sm:max-w-fit overflow-x-auto hide-scrollbar px-2">
        <div className="flex items-center p-1.5 bg-black/30 backdrop-blur-xl border border-white/8 rounded-full shadow-sm mx-auto min-w-max">
          {items.map((item) => {
            const isActive = pathname === item.path || (item.path === "/report_data/financial_position" && pathname === "/report_data/financial_position");
