...
import { ratiosFinancialPositionDataByYear, years } from "@/components/Pages/Ratios/ratiosData";
import EChartsLineChart from "@/components/Ui/Charts/EChartsLineChart";

function buildSeries(label: string) {
  return years.map((y) => {
    const row = ratiosFinancialPositionDataByYear[y].find((r) => r.label === label);
