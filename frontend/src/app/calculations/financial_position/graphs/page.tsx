...
import { calculationsFinancialPositionDataByYear, years } from "@/components/Pages/Calculations/calculationsData";
import EChartsLineChart from "@/components/Ui/Charts/EChartsLineChart";

function buildSeries(label: string) {
  return years.map((y) => {
    const row = calculationsFinancialPositionDataByYear[y].find((r) => r.label === label);
