...
export default function CalculationsFinancialPositionGraphsPage() {
        ...
        {!hasSymbol && !symbol && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-lg font-bold text-slate-300 font-inter">
              Financial Position Calculation Charts
            </p>
            <p className="mt-2 text-sm text-[#475569] font-inter">
              Select a company to view financial position calculation trend charts.
            </p>
          </div>
        )}
            ...
            {symbol && (
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs text-[#475569] font-inter">
                  Financial Position Calculations · Trend Analysis
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {loading
                ? Array.from({ length: 4 }).map((_, i) => (
                  <GraphCard key={i} title="Loading…" color="#38BDF8" loading>
                    <div />
                  </GraphCard>
                ))
                : series.map((s) => (
                  <GraphCard key={s.label} title={s.label} color={s.color}>
                    <EChartsLineChart
                      data={s.data}
                      color={s.color}
                      height={280}
